...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    i, is_sowsad, sum_roll_dice = 0, 0, 0
    while i != num_rolls:
        single_roll_dice = dice()
        if single_roll_dice == 1:
            is_sowsad = 1
        else:
            sum_roll_dice += single_roll_dice
        i += 1
    return is_sowsad or sum_roll_dice
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    three_mul_absolute_difference = 3 * abs(opponent_tens_digit - player_ones_digit)
    if three_mul_absolute_difference <= 1:
        return 1
    else:
        return three_mul_absolute_difference
    # END PROBLEM 2
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def strategy_function(score, opponent_score):
        return n
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while i < goal:
        j = 0
        while j < goal:
            if not strategy(i, j) == initial_value:
                return False
            j += 1
        i += 1
    return True
...
def make_averaged(original_function, times_called=1000):
    ...
    def averaged_function(*args):
        times, total_dice = 0, 0
        while times < times_called:
            single_dice = original_function(*args)
            total_dice += single_dice
            times += 1
        averaged_dice = total_dice / times_called
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    num_rools, max_scoring = 1, 0
    while num_rools <= 10:
        averaged_dice = make_averaged(roll_dice, times_called)
        average_scoring = averaged_dice(num_rools, dice)
        if average_scoring > max_scoring:
            best_num_rolls = num_rools
            max_scoring = average_scoring
        num_rools += 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    zero_dice_gain_score = sus_update(0, score, opponent_score) - score
    if zero_dice_gain_score >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
